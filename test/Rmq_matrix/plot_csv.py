...
def process_single_csv(args):
    ...
    try:
        ...
        match = re.search(
            r"Rmq_matrix_N_(\d+)_Nq_(\d+)_basis_(GLL|GL)_quadrature_(GLL|GL)",
            filepath.name,
        )
        if match is None:
            raise ValueError(
                "Expected filename pattern Rmq_matrix_N_<N>_Nq_<Nq>_basis_<GL|GLL>_quadrature_<GL|GLL>.csv"
            )
        ...
        title = filepath.stem.replace("Rmq_matrix_", "").replace("_", " ")
        matrix_ax.set_title(f"Rmq Matrix: {title}", fontsize=10, pad=10)
        matrix_ax.set_xlabel("Basis Index (j)")
        matrix_ax.set_ylabel("Basis Index (i)")

        # Plot all Lagrange basis polynomials associated with this Rmq matrix.
...
def plot_all_Rmq_matrices(
    ...
    csv_files = list(src_path.glob("Rmq_matrix_*.csv"))

    if not csv_files:
        print(f"No Rmq matrix CSV files found in '{src_path.resolve()}'.")
        return
...
if __name__ == "__main__":
    plot_all_Rmq_matrices(max_workers=32, cmap="magma")
