...
def process_single_csv(args):
    ...
    try:
        ...
        match = re.search(
            r"Sjq_matrix_N_(\d+)_Nq_(\d+)_basis_(GLL|GL)_quadrature_(GLL|GL)",
            filepath.name,
        )
        if match is None:
            raise ValueError(
                "Expected filename pattern Sjq_matrix_N_<N>_Nq_<Nq>_basis_<GL|GLL>_quadrature_<GL|GLL>.csv"
            )
        ...
        title = filepath.stem.replace("Sjq_matrix_", "").replace("_", " ")
        matrix_ax.set_title(f"Sjq Matrix: {title}", fontsize=10, pad=10)
        matrix_ax.set_ylabel("Basis Index (j)")
        matrix_ax.set_xlabel("quadrature node Index (q)")

        # Plot all Lagrange basis polynomials associated with this Sjq matrix.
...
def plot_all_Sjq_matrices(
    ...
    csv_files = list(src_path.glob("Sjq_matrix_*.csv"))

    if not csv_files:
        print(f"No Sjq matrix CSV files found in '{src_path.resolve()}'.")
        return
...
if __name__ == "__main__":
    plot_all_Sjq_matrices(max_workers=32, cmap="magma")
